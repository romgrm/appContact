...
const reducer = (state, action) => {
    ...
    switch(action.type) {
        case 'DELETE_CONTACT':
            return{
                contacts: state.contacts.filter(current => current.id !== action.payload) // on return la fonction similaire qu'on avait fait dans Liste.js pour delete un contact sauf qu'a la fin c'est pas juste 'id' mais action.payload, payload signifiant lorsque l'action est efectuée
            }
    }
} 
