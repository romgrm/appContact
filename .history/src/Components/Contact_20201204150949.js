...
export default class Contact extends Component {
    ...
    // La fonction deleteContact va prendre en paramètre l'id de notre contact (présent dans le state global de Context.js) ainsi que le dispatch (permet de récupérer les actions émises par le Reducer)
    deleteContact = (id, dispatch) => {
        dispatch({type: 'DELETE_CONTACT', payload: id}) // on éxécute le dispatch en lui mettant le type d'action a effectuer (on reprend l'action DELETE_CONTACT créée dans le switch de notre Reducer) et le trigger/déclencher Payload qui est l'id de notre contact à delete
    }
