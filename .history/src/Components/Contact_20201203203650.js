    ...
    render() {
        ...
        return(
            <Consumer>
                ...
                onClick={() => this.deleteContact(this.props.id, value.dispatch)}></i> {/** ici on utilise notre Croix pour effectué une action au clic. Cette action va appeler la fonction deleteContact() qui va prendre en paramètre l'id de notre contact (qui est dans le state) ainsi que l'action à executée (DELETE_CONTACT) présent dans le Reducer, envoyée grâce au Dispatch, lui même présent dans le state */}
                </p>
